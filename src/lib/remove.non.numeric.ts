 ...
 */
export function removeNonNumericCharacters<T extends Nil | string>(input?: T) {
  type Result = T extends Nil ? undefined : string;
  return (isNil(input) ? undefined : input.replace(/[^0-9]/g, '')) as Result;
}
